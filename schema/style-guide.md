...
- **No external references in pattern text.** No hyperlinks of any kind, and no WCAG criterion identifiers. An agent does not follow a link, and a criterion number is a reason rather than a requirement, so both spend retrieval budget on every lookup to deliver something nothing reads. The corpus is WCAG-derived and `CONTRIBUTING.md` says so; individual requirements do not restate it. Where a criterion genuinely changes what gets built, state the behavior it requires instead of citing it. This governs `components/*.md` and `global_rules.md` only: `intro.md`, `release-notes.md`, and `qa-catalog.json` are not pattern text and keep theirs.
