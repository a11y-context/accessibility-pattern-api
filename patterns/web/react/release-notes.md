...
- **`schema/style-guide.md` now states the rule**, under Prose and punctuation: no hyperlinks and no criterion identifiers in pattern text, with the three non-pattern files named as exceptions. The hyperlink half was enforced in 0.4.1 on the iOS side and never written down, which is how it would have come back.
...
- **Acceptance Checks removed from all 33 published web/react patterns**, and the section is no longer part of the format: `schema/style-guide.md` drops it from the section order and deletes its subsection, `schema/pattern-template.md` drops both its guidance and its slot in the skeleton, and `CONTRIBUTING.md` no longer requires it. The section reached agents in full, because the skills sync ships each pattern's whole `.md` rather than only its catalog entry, so it was spending retrieval budget on every lookup to restate requirements the Must Haves and Don'ts already carry. **No per-pattern version bumps:** no requirement changed in any of the 33, which puts this in the same class as a corpus-wide typography sweep. The Foundations rules in `global_rules.md` and the 19 `ios/swiftui` patterns keep theirs for now.
- **`toast.action` → 0.2.0. The persistent variant is gone.** The pattern previously branched: a message whose action was reachable elsewhere timed out, and a message carrying the only route to its action stayed until the user acted. The second branch is deleted, and its case now redirects to `banner.basic`. The WCAG 2.2.1 analysis that produced the branch still holds, which is that a vanishing message only sets a time limit when the user has no other way to perform the function; what changed is the response to it. A message that must not disappear is not a transient message, and building one out of this pattern produces a banner wearing a toast's clothes. So availability elsewhere is now a Must Have and a selection criterion rather than a condition on the timer.
- **The message is the landmark, rather than living inside one.** The draft wrapped every message in a container carrying `role="region"`, which is the right shape only when messages stack. For the common case of one message it is a wrapper that exists to hold a single child, and it made the pattern read as though a queue were mandatory. A single message now carries `role="region"` and its accessible name directly. Stacking moves to Customizable: when several messages can be on screen, the role moves to one container and the per-message role is dropped, so the page still exposes one landmark. A Don't names the failure that sits between the two arrangements, which is a landmark per message filling the rotor's landmark list with duplicates of one name. Net effect on the Golden Pattern is 118 lines down to 104, with no queue machinery in the default and Must Haves down from 24 to 21.
- **`toast.action` states its own weakness in the preamble.** Focus never moves to the message, which is what keeps it from interrupting the user and is also what puts its action out of reach. Screen reader users can navigate to it as a landmark; sighted keyboard-only users have no equivalent, are never told a shortcut exists, and may not reach the button before the timer fires. The pattern now says so above its selection criteria, and recommends against use where another pattern fits.
- **The `dialog.alert` boundary turns on criticality rather than urgency, in both toasts.** `toast.basic` → 0.3.0. Both previously read "Do not use when the message is urgent and must interrupt the user immediately", which sorts on the wrong axis: "Added to your watchlist" is time-sensitive and unimportant, and a toast is exactly right for it. What separates the two is whether the user is expected to respond.
- **Three Foundations restatements cut, and one requirement stated three times.** Both toasts repeated `global.motion` verbatim on when a dismissed element leaves the accessibility tree, and `toast.action` also repeated its focus-before-removal rule. `toast.basic` carried its live-region mounting requirement in Roles & structure, again in State & properties, and again in Don'ts; the middle copy is gone.
- **`global.non-text-contrast` applied to both toasts**, which are floating surfaces whose boundary against the content behind them needs 3:1. Five other patterns carry the rule; these two cited `global.forced-colors` and skipped it.
- **`toast.basic` housekeeping.** The focus-state requirement is now conditional on a dismiss control existing, since the control is optional. The Don't against moving focus to the message moved to the top of that section.
- **Sidebar order pinned.** `toast.basic` ("Toast") now precedes `toast.action` ("Toast with Action"), the reverse of alphabetical, following the `select.native` / `select.basic` precedent. The base pattern should be read before the variant.

`qa-catalog.json` is unchanged, but `/qa-derive` moved off Acceptance Checks: rules still come from Must Haves and Don'ts, `Customizable` is now read on every derive to bound what a check may flag, and `runtime_check` wording is written from the requirement itself.

## 0.24.0 — 2026-09-16

**The action-bearing transient message, and the older text-only one brought up to the current Golden Pattern rules. The pair splits on whether the control does something the timer cannot do for the user.**

- **New pattern `toast.action` ("Toast with Action")**, beta at 0.1.0. The transient message carrying exactly one action, such as Undo, Retry, or View. A survey of fifteen design systems settled the naming question before anything was written: only Base Web ships "snackbar" and "toast" as distinct components, MUI documents them as the same thing ("Snackbars (also known as toasts)"), and eleven systems use one term for the whole category without ever mentioning the other. They are one pattern under two names, so the ID follows the corpus taxonomy and `snackbar` leads the aliases.
- **Auto-dismissal is decided by where the action lives, and WCAG 2.2.1 is what decides it.** The field's majority position is that an action and a dismiss timer conflict, resolved by never auto-dismissing an actionable message at all (Spectrum, Base Web, Material, GitLab, and Carbon each arrive there independently). This pattern takes the narrower route the Understanding document for Timing Adjustable actually supports: a disappearing message sets a time limit only when "the user has no other means of discovering the same information (or performing the same function)". So a message whose action is reachable elsewhere may time out, and a message carrying the only route to its action does not dismiss itself. The two branches are co-equal Must Haves rather than a rider on one of them, because a conditional stated as an implication of a different requirement is a conditional a generating agent skips. Pausing on hover and on focus is required on its own merits and is explicitly not the conformance mechanism: it satisfies none of Turn off, Adjust, or Extend.
- **Announce in one place, interact in another.** The live region holds text only and names both the action and the route to it, while the visible message and its real `<button>` sit in a separately mounted landmark region. Interactive content inside a live region is announced as flattened text, so a button nested there is spoken as part of the message with nothing marking it activatable. ARIA carries no prohibition on this, and the role definitions were read directly rather than summarized to confirm it, so the requirement is stated as the observable behavior instead of as a citation the spec does not support.
- **One region holds the queue, not one region per message.** Caught in review: an architecture where the component renders both the landmark and its single message produces a landmark per message, filling the rotor's landmark list with duplicates of one name. The region is now the container and the messages are its children, which also makes the focus-moves-to-the-next-message requirement expressible rather than theoretical. Timers bank their elapsed time, so a paused message resumes where it stopped, and a reader who lingers is not given less time than one who never looked at it.
- **`toast.basic` modernized to 0.2.0.** Its Golden Pattern predated the 0.22.0 strategy and exported three things including a `ToastDemo`; it is now a single `Toast` export. Two Must Haves phrased as prohibitions moved to Don'ts, negation-only Don'ts were cut, and a Use When bullet describing a capability the pattern lacks was replaced with one that routes. One substantive addition: a dismiss control is a sibling of the announcing element rather than a descendant of it, which is the same defect the new pattern guards against. Both of its `snackbar` redirects, which pointed at an ID that was never going to exist, now point at `toast.action`.
- **The boundary between the two.** `toast.basic` owns the message with nothing to act on; `toast.action` owns the one carrying a control the timer cannot operate on the user's behalf. That is why only the action-bearing pattern requires a landmark route: a dismiss control is a convenience when the message removes itself anyway, and an Undo is not.
- **Forward reference seeded:** `banner.basic`, for a persistent page-level condition rather than the outcome of an action the user just took. Declared in the authoring backlog rather than left dangling. The stale `snackbar` entry in that backlog is struck, superseded by this release.

`qa-catalog.json` is unchanged: `toast.action` is queued for a derive batch, and `toast.basic` needs re-deriving against its rewritten requirements.
