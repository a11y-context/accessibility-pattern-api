...
The corpus is one source of truth with many consumption modes: fetch it over HTTP, copy it locally, query it through an MCP server, or index it into an enterprise RAG system. Browse it now: [React (Web) patterns](/web/react).

## Why it exists

AI coding assistants generate inaccessible UI by default. In [Microsoft's A11y LLM Eval Report](https://microsoft.github.io/a11y-llm-eval-report/), models generating UI code with no accessibility instructions passed automated WCAG checks **12% of the time on average**; the best-performing model managed 25%. And automated checks only catch about half of real accessibility failures — actual performance is worse than those numbers suggest.
...
**[For AI coding agents](/getting-started/ai-coding-agents/)** — the primary use. An always-on rule directs the agent to identify the components it is about to build, retrieve only those patterns plus the global rules, and apply them before writing code.
