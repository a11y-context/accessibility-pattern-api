...
**The corpus is public, structured, and Apache-2.0 licensed.** Every pattern is a self-contained markdown file. `patterns.json` is a machine-readable catalog. Everything you need to build a custom integration or point an enterprise retrieval system at the corpus is already available:
...
## Enterprise search & vector-DB integrations
...
## The RAG skill and its config

Indexing the corpus into your vector DB is only the retrieval half. You still need the **brain** — a skill that gets invoked, decomposes the request, selects patterns via `Use When` / `Do Not Use When`, and *then* queries your index. That's the same skill as every other integration; only its retrieval step differs.

The catch: the HTTP and Local skills have their address baked in (our URL, a bundled dir), but a RAG skill can't — your vector DB is your infrastructure, unknown when we author the skill. So the RAG skill variant ships with a **config file** you fill in at install:
...
:::note
The RAG skill variant + config scaffold is in development. Today, you can index the corpus (above) and add the selection + invocation instructions to your agent's system prompt or tool-selection layer manually — the [skill ZIPs](./downloads) are a working reference for the wording. Track the packaged RAG skill in the [repo issues](https://github.com/a11y-context/accessibility-pattern-api/issues).
:::
