...
What changes between integrations is the *retrieval* step: where the skill goes to fetch the patterns it selected. So each integration is a skill *variant* whose retrieval step targets a different mechanism.

| Retrieval mechanism | The skill's retrieval step | Where the address lives | Status |
|---|---|---|---|
| **[HTTP](./http)** | fetches pattern pages from this site | baked into the skill (our URL) | ✅ Available |
| **[Local](./local)** | reads a bundled copy from disk | baked into the skill (bundled dir) | ✅ Available |
...
- **Most teams: [Skill — HTTP](./http).** Best measured quality at the lowest complexity — nothing to vendor locally, always current, invocation handled by the skill's description. Recommended starting point.
- **Offline or air-gapped: [Skill — Local](./local).** Fastest retrieval, no network; refresh periodically.
